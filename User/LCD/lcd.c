...
//WKS GD32F427ZGT6?????
//2.8??/3.5??/4.3??/ TFT???????
//???????IC??????:NT35310/NT35510/ST7796/ST7789
//?汾??V1.0
//////////////////////////////////////////////////////////////////////////////////
...
{
 	u16 r=0,g=0,b=0;
	if(x>=lcddev.width||y>=lcddev.height)return 0;	//???????Χ,??????		   
	LCD_SetCursor(x,y);
    if (lcddev.id == 0X5510)    //5510 ?????GRAM???
    {
        LCD_WR_REG(0X2E00);
    }
    else                        //????IC(7796/5310/7789)?????GRAM???
    {
        LCD_WR_REG(0X2E);
    }
    ...
    //7796/5310/5510/7789 ???2?ζ???
	opt_delay(2);
    b = LCD_RD_DATA();
    g = r & 0XFF;               //???? 7796/5310/5510/7789, ????ζ??????RG???,R???,G???,???8λ
    g <<= 8;
		if (lcddev.id == 0X7796)    
    {
        return  r;              //7796?????????
    }  
    return (((r >> 11) << 11) | ((g >> 10) << 5) | (b >> 11));  // 5310/5510/7789 ????????????
}
...
void LCD_Scan_Dir(u8 dir)
{
    ...
    if (lcddev.id == 0X7796 || lcddev.id == 0X7789)   //7796 & 7789 ?????BGRλ
    {
        regval |= 0X08;
    }
...
void LCD_Display_Dir(u8 dir)
{
    ...
    if (dir == 0)           //????
    {
        ...
        if (lcddev.id == 0x5510)
        {
            lcddev.wramcmd = 0X2C00;   //????д??GRAM?????
            lcddev.setxcmd = 0X2A00;   //????дX???????
            lcddev.setycmd = 0X2B00;   //????дY???????
...
{ 	  
  ...
  __HAL_RCC_GPIOG_CLK_ENABLE();			//???GPIOG???  
	...
	HAL_GPIO_Init(GPIOG,&GPIO_Initure);     //PG6,LCD??λ????
	...
	TFTSRAM_Handler.Init.MemoryDataWidth=FSMC_NORSRAM_MEM_BUS_WIDTH_16; //16λ??????
	TFTSRAM_Handler.Init.BurstAccessMode=FSMC_BURST_ACCESS_MODE_DISABLE; //?????????????,???????????洢????Ч,???δ???
	TFTSRAM_Handler.Init.WaitSignalPolarity=FSMC_WAIT_SIGNAL_POLARITY_LOW;//??????????,???????????????????
	TFTSRAM_Handler.Init.WaitSignalActive=FSMC_WAIT_TIMING_BEFORE_WS;   //?洢?????????????????????????????????????????NWAIT
	TFTSRAM_Handler.Init.WriteOperation=FSMC_WRITE_OPERATION_ENABLE;    //?洢??д???
	TFTSRAM_Handler.Init.WaitSignal=FSMC_WAIT_SIGNAL_DISABLE;           //??????λ,???δ???
	TFTSRAM_Handler.Init.ExtendedMode=FSMC_EXTENDED_MODE_ENABLE;        //??д??ò???????
	TFTSRAM_Handler.Init.AsynchronousWait=FSMC_ASYNCHRONOUS_WAIT_DISABLE;//???????????????????????,???δ???
	TFTSRAM_Handler.Init.WriteBurst=FSMC_WRITE_BURST_DISABLE;           //??????д
	TFTSRAM_Handler.Init.ContinuousClock=FSMC_CONTINUOUS_CLOCK_SYNC_ASYNC;
    
	//FMC????????????
	FSMC_ReadWriteTim.AddressSetupTime=0x0F;       	//??????????ADDSET???15??HCLK 1/168M=6ns*15=90ns
	FSMC_ReadWriteTim.AddressHoldTime=0;            //??????????ADDHLD????Aδ???	
	FSMC_ReadWriteTim.DataSetupTime=60;				      //??????????(DATAST)?60??HCLK	=6*60=360ns
	FSMC_ReadWriteTim.AccessMode=FSMC_ACCESS_MODE_A;//??A
	//FMCд??????????
	FSMC_WriteTim.BusTurnAroundDuration=0;			    //?????????γ???????0????????????????????????????????4????????????в?????
	...
	HAL_SRAM_Init(&TFTSRAM_Handler,&FSMC_ReadWriteTim,&FSMC_WriteTim);	//?????SRAM????
  ...
  //LCD??λ
	...
	lcddev.id=LCD_RD_DATA();   	//???77								   
	lcddev.id<<=8;
	lcddev.id|=LCD_RD_DATA();  	//???96 	   			   
	if(lcddev.id!=0X7796)		//????ST7796,????????????ST7789
	{
        LCD_WR_REG(0X04);
        lcddev.id = LCD_RD_DATA();      //dummy read
        ...
        if (lcddev.id != 0x7789)        //?????ST7789,?????????NT35310
        {
            ...
            if(lcddev.id!=0X5310)		//?????NT35310,????????????NT35510
            {
                ...
                LCD_WR_REG(0xC500);             //???ID??8λ
                lcddev.id = LCD_RD_DATA();      //????0X55
                lcddev.id <<= 8;

                LCD_WR_REG(0xC501);             //???ID??8λ
                lcddev.id |= LCD_RD_DATA();     //????0X10
                delay_ms(5);
    ...
    //???????????,????
	  if(lcddev.id==0X7796||lcddev.id==0x7789||lcddev.id==0x5310||lcddev.id==0X5510)//?????????IC,??????WR???????
    {
        //????????д???????????????
        ...
        {
            FSMC_Bank1E->BWTR[6]|=4<<8; //??????????(DATAST)?6ns*5??HCLK=30ns
        }
        else
        {
...
void LVGL_Fill(u16 sx,u16 sy,u16 ex,u16 ey,u16 *color)
{
    u16 height,width;
    u16 i,j;
